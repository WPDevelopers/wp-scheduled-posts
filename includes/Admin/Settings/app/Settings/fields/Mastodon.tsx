...
const Mastodon = (props) => {
    ...
    const cachedLocalData = JSON.parse(localStorage.getItem('mastodon'));
    ...
    const [profileStatus, setProfileStatus] = useState(
        builderContext?.savedValues?.mastodon_profile_status
    );
    ...
    useEffect(() => {
        onChange({
            target: {
                ...
                name: 'mastodon_profile_status',
                value: profileStatus,
            },
            });
        }, [profileStatus]);
        const [isErrorMessage, setIsErrorMessage] = useState(false)
        localStorage.setItem('mastodon',JSON.stringify(cachedStatus));
    ...
    const handleMastodonFetchProfile = async (redirectURI, appID, appSecret, platform, openIDConnect = false) => {
        ...
        if( responseData?.data?.name && responseData.success )  {
            ...
            SweetAlertToaster({
                type : 'error',
                title : responseData?.data?.message ?  responseData?.data.message : __( 'Please check your instance URL and access token and try again.', 'wp-scheduled-posts' ),
            }).fire();
        }
    };

    const addMastodonProfile = (event, profileInfo) => {
        if( event.target.checked ) {
            // free
            // @ts-ignore
            ...
            <Modal
                ...
                <button
                    ...
                    <ApiCredentialsForm
                        props={props}
                        platform={platform}
                        requestHandler={ handleMastodonFetchProfile }
                        appInfo={appInfo}
                    />
            </Modal>
            {/* @ts-ignore */}
