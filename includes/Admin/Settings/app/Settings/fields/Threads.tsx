...
const Threads = (props) => {
    ...
    const cachedLocalData = JSON.parse(localStorage.getItem('threads'));
    ...
    const [profileStatus, setProfileStatus] = useState(
        builderContext?.savedValues?.threads_profile_status
    );
    ...
    useEffect(() => {
        onChange({
            target: {
                ...
                name: 'threads_profile_status',
                value: profileStatus,
            },
            });    
        }, [profileStatus]);
        const [isErrorMessage, setIsErrorMessage] = useState(false)
        localStorage.setItem('threads',JSON.stringify(cachedStatus));
    ...
    const handleThreadsFetchProfile = async (redirectURI, appID, appSecret, platform, openIDConnect = false) => {
    ...
    const addThreadsProfile = (event, profileInfo) => {
        if( event.target.checked ) {
            // free
            // @ts-ignore
            ...
            <Modal
                ...
                <button
                    ...
                    <ApiCredentialsForm
                        props={props}
                        platform={platform}
                        requestHandler={ handleThreadsFetchProfile }
                        appInfo={appInfo}
                    />
            </Modal>
            {/* @ts-ignore */}
