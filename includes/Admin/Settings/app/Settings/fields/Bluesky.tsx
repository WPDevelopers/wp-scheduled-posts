...
const Bluesky = (props) => {
    ...
    const cachedLocalData = JSON.parse(localStorage.getItem('bluesky'));
    ...
    const [profileStatus, setProfileStatus] = useState(
        builderContext?.savedValues?.bluesky_profile_status
    );
    ...
    useEffect(() => {
        onChange({
            target: {
                ...
                name: 'bluesky_profile_status',
                value: profileStatus,
            },
            });
        }, [profileStatus]);
        const [isErrorMessage, setIsErrorMessage] = useState(false)
        localStorage.setItem('bluesky',JSON.stringify(cachedStatus));
    ...
    const handleBlueskyFetchProfile = async (redirectURI, appID, appSecret, platform, openIDConnect = false) => {
        ...
        if( responseData?.data?.name && responseData.success )  {
            ...
            SweetAlertToaster({
                type : 'error',
                title : responseData?.data?.message ?  responseData?.data.message : __( 'Please check your handle and App Password and try again.', 'wp-scheduled-posts' ),
            }).fire();
        }
    };

    const addBlueskyProfile = (event, profileInfo) => {
        if( event.target.checked ) {
            // free
            // @ts-ignore
            ...
            <Modal
                ...
                <button
                    ...
                    <ApiCredentialsForm
                        props={props}
                        platform={platform}
                        requestHandler={ handleBlueskyFetchProfile }
                        appInfo={appInfo}
                    />
            </Modal>
            {/* @ts-ignore */}
