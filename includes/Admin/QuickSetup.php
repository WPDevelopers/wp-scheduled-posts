...
class QuickSetup
{
    ...
    public static function load()
    {
         ...
         * Welcome Screen
         */
        add_action('wpscp_pro_qsw_welcomescreen', [__CLASS__, 'wpscp_pro_qsw_welcomescreen_markup']);

        /**
         * Pro Feature
         */
        add_action('wpscp_pro_qsw_profeature_list', [__CLASS__, 'wpscp_pro_feature_list_markup']);

        self::quick_setup_config();
    }

    public static function quick_setup_config()
    {
        // step one
        self::setSection(array(
    ...
    public static function setup_wizard_scripts($hook)
    {
        ...
        if (is_admin() && \WPSP\Helper::plugin_page_hook_suffix($current_post_type, $hook)) {
            wp_enqueue_style('wpscp-setup-wizard', WPSP_ASSETS_URI . 'css/wpscp-setup-wizard.css', array(), filemtime(WPSP_ASSETS_DIR_PATH . 'css/wpscp-setup-wizard.css'), 'all');
            wp_enqueue_script('wpscp-setup-wizard', WPSP_ASSETS_URI . 'js/wpscp-setup-wizard.js', array('jquery'), filemtime(WPSP_ASSETS_DIR_PATH . 'js/wpscp-setup-wizard.js'), false);
        }
    }
...
?>
    ...
    public static function wpscp_pro_qsw_welcomescreen_markup()
    {
    ...
    public static function wpscp_pro_feature_list_markup()
    {
    ?>
        <td>
            <!-- auto scheudled -->
            <div class="wpscp-checkbox wpscp-pro-feature-checkbox" data-id="module_auto_scheduled">
                ...
                <p class="wpscp-module-title"><?php esc_html_e('Auto Scheduler', 'wp-scheduled-posts'); ?>
                    <a rel="nofollow" target="_blank" href="https://wpdeveloper.net/docs/wp-scheduled-posts/how-does-auto-scheduler-work/">
                        <img width="6px" src="<?php echo esc_url(WPSP_ASSETS_URI .  'images/question.svg'); ?>" alt="wp scheduled posts" />
                    </a>
