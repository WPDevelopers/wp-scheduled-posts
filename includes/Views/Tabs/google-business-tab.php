<?php
/**
 * Google Business Tab Content for Social Message Modal
 * 
 * @package WPSP
 */
...
// Get Google Business profiles and selected data
$googleBusinessProfiles = \WPSP\Helper::get_settings('google_business_profile_list');
$selected_google_business_profiles = isset($custom_templates['google_business']['profiles']) ? $custom_templates['google_business']['profiles'] : array();
$google_business_template = isset($custom_templates['google_business']['template']) ? $custom_templates['google_business']['template'] : '';
$google_business_is_global = isset($custom_templates['google_business']['is_global']) ? $custom_templates['google_business']['is_global'] : '';
?>

<!-- Google Business Profile Section -->
<div class="wpsp-profile-selection-area-wrapper" id="wpsp-profile-google_business" style="display: none;">
    <div class="selected-profile-area">
        <ul>
            <?php
                if ( ! empty( $googleBusinessProfiles ) && is_array( $googleBusinessProfiles ) ) :
                    foreach ( $googleBusinessProfiles as $profile ) :
    ...
    <div class="wpsp-profile-selection-dropdown">
        <div class="wpsp-profile-selection-dropdown-item">
            <?php if ( ! empty( $googleBusinessProfiles ) && is_array( $googleBusinessProfiles ) ) : ?>
                <?php foreach ( $googleBusinessProfiles as $profile ) : 
                     $thumbnail_url = get_safe_thumbnail_url($profile, $this->autho_logo);
                ?>
                <div class="wpsp-profile-card" data-profile-id="<?php echo esc_attr( $profile->id ); ?>" data-profile-name="<?php echo esc_attr( $profile->name ); ?>" data-profile-img="<?php echo esc_url( $thumbnail_url ); ?>">
                    ...
                    <div class="wpsp-profile-checkbox">
                        <input 
                            type="checkbox" 
                            class="wpsp-modal-profile-checkbox wpsp-google-business-checkbox" 
                            name="google_business_profiles[]"
