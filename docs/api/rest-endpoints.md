...
| `POST` | `wp-scheduled-posts/v1/update-refresh-token` | `wpsp_update_refresh_token` | Params: `platform`, `item`. Delegates to `Social\ReconnectHandler::handleProfileReconnect()`. Success additionally requires the refreshed credential to have been persisted; `update_access_token()` returns a `WP_Error` for a missing/malformed settings option, an unknown profile id, or a failed write, and that becomes a 500. Authorization is in the `permission_callback`: no `edit_posts` is 401/403 via `rest_authorization_required_code()`, failing `Helper::is_user_allow()` is 403. A failed reconnect returns a `WP_Error` (400 for a bad request, 502 for a transport or Instagram-side failure), never a 200 carrying `success: false`. |
...
| `POST` | `wp-scheduled-posts/v1/update-settings/{post_id}` | `publish_immediately` | `post_id`, `publish_immediately_current_date` (bool), `publish_immediately_future_date` (bool) | Exactly one action flag is required: neither or both is a 400. A post that is not dated in the future is a 400 for the future-date action; a failed write is a 500. The intent meta is only recorded, and `wpsp_pro_update_post` only fired, after the post has actually been published. `{ success: true, message, data: { post_status } }` |
| `DELETE` | `wp-scheduled-posts/v1/update-settings/{post_id}` | `clear_publish_immediately` | `post_id` | Requires an active intent, using the same equality rule the `GET` route reports it by, so it never reschedules a post this feature did not publish. An absent or stale intent is an idempotent no-op (a stale row is still tidied away) and the post status is untouched. With an active intent the meta is deleted and, when the date has not passed, the post returns to `future`. A failed reschedule restores the exact previous meta and answers 500. `{ success: true, message, data: { post_status, prevent_future_post: false, rescheduled } }` |
