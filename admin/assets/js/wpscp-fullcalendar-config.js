document.addEventListener("DOMContentLoaded", function() {
  ...
  function wpscp_calender_ajax_request(obj) {
    ...
    jQuery.post(wpscp_calendar_ajax_object.ajax_url, data, function(
      response,
      status
    ) {
      if (status == "success") {
        var jsonData =
          response !== null && response !== "" ? JSON.parse(response) : [];
        if (obj.type == "addEvent" && obj.post_status == "Scheduled") {
          ...
          wpscp_calendar_notifi({
            ...
            post_status:
              jsonData.length > 0 ? jsonData[0].post_status : "future",
            post_date: obj.date
          });
  ...
  function wpscpGetPostTypeNameSkipUnderScore(oldRestUrl, month, year) {
    ...
    if (year !== null) {
      updateRestUrl = updateRestUrl.replace(/year=[0-9]{4}/i, "year=" + year);
    }
